...
export function AddExerciseDialog({ 
  ...
  const { control, register, handleSubmit, reset, watch } = useForm<ExerciseFormData>({
    ...
    if (isOpen) {
      if (exerciseToEdit) {
        reset({
          ...
          warmup: exerciseToEdit.warmup || inferWarmupTemplate(exerciseToEdit.name),
        });
      } else {
        const inferred = inferWarmupTemplate(exerciseName || '');
        reset({ 
          name: exerciseName || '', 
  ...
  }, [exerciseToEdit, reset, isOpen, exerciseName]);
