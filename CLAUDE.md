...
- Mobile keyboard: `position: fixed` resolves against the LAYOUT viewport, which iOS never shrinks for the keyboard, and `body{overflow:hidden}` does NOT stop iOS touch-scroll chaining. `CoachChatSheet` therefore sizes its panel from `useVisualViewport` (`visualViewport.offsetTop`/`.height`) and pins the body (`position:fixed` + negative `top`, scroll restored on close). A `dvh`/centered panel slid under the keyboard and the page behind kept scrolling. Also: any mobile input forced to `text-sm` (<16px) makes iOS zoom on focus, which re-triggers the whole viewport dance — keep the Textarea's base `text-base md:text-sm` (fixed 2026-08)
